...
fn arrange(arranged_map: &mut HashSet<C3>, positions: &Vec<C3>) -> Option<C3> {
    for rot in 0..24 {
...
fn main() {
    ...
    while !scanners.is_empty() {
        for i in (0..scanners.len()).rev() {
            if let Some(d) = arrange(&mut arranged_map, &scanners[i]) {
                dists.push(d);
                scanners.swap_remove(i);
            }
        }
    }
    let result = dists.iter()
                      .flat_map(|x| dists.iter().map(move |y| (x.clone(), y.clone())))
