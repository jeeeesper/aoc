...
fn count_paths<'a>(graph : &HashMap::<&'a str, Vec<&'a str>>, mut visited : HashSet<&'a str>, v : &'a str) -> u32 {
    if v == "end" {
        return 1;
    }
    ...
    let mut sum = 0;
    for w in &graph[&v] {
        sum += count_paths(graph, visited.clone(), w);
    }
    return sum;
}
...
fn main() {
    ...
    println!("{}", count_paths(&graph, visited, "start"));
}
